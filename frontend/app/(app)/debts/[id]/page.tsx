...
export default function DebtDetailPage() {
  ...
  const payments = [...debt.payments].sort((a, b) => new Date(b.createdAt ?? 0).getTime() - new Date(a.createdAt ?? 0).getTime());
  const rows = payments.map((payment, index) => ({
    payment,
    balance: debt.amount - payments.slice(0, index + 1).reduce((sum, entry) => sum + entry.amount, 0),
  }));
